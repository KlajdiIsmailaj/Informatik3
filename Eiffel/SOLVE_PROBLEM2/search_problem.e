...
feature{NONE}
     -- when creating this object it recieves the input form the console and the param will be used in the various routines below
       ...
       invalid_create(input_console:INTEGER)
       -- invalid_create(number)
 ...
 feature{ANY}-- attributes-----------------------------------------
            ...
            T:detachable BINARY_TREE3[G]
            d_node:detachable NODE3[G]
            exist:BOOLEAN
            number2:detachable G
            	number:INTEGER
...
--feature{ANY}-----setter-----------------
	 ...
	 set_tree(Tre:detachable BINARY_TREE3[INTEGER])
            local
...
---------------------------------------------------------------------------------------------------
     ...
     -- rename is_solvable as isSolvable
           ...
           is_solvable
              ...
              local
                  ...
                  do
                   	...
                   	check attached d_node as e_node then
                     if attached current.number as console then


                     	if e_node.get_value > current.console and e_node.get_right_child/=Void then

                     		 directly_solvable:=TRUE

                     		 elseif e_node.get_value <current.number  and e_node.get_left_child /=Void then
